...
if (isset($_POST['submit'])) {
    ...
    if (!empty($name)) {
        ...
        $warning_msg[] = "Name update failed!";
    }
    ...
    if (!empty($email)) {
        ...
        $warning_msg[] = "Email update failed!";
    }
    ...
    if ($empty_old != 1) {
        $verify_old_pass = password_verify($_POST['old_pass'], $pre_pass);
        if ($verify_old_pass == 1) {
            if ($c_pass == 1) {
                if ($empty_new != 1) {
                    $udpate_pass = $conn->prepare("UPDATE `users` SET password = ? WHERE id = ?");
                    $udpate_pass->execute([$new_pass, $user_id]);
            ...
            $warning_msg[] = 'The old password is not correct!';
        }
    } else {
        $warning_msg[] = 'Please enter your password!';
    }
    ...
    <section class="account-form">
        <form action="" method="post" enctype="multipart/form-data">
            ...
            if ($fetch_profile['image'] != '') { ?>
                ...
                <input type="submit" value="delete image" class="delete-btn" onclick="return confirm('Delete this image?')" name="delete_image">
            <?php }; ?>
            <p class="placehoder">Profile picture</p>
